...
public class WordBreak {
    ...
    //dictonary is a set.
    //unmatching string are in a set.
    //inputString is truncated each time.
    public static boolean findUsingDP(String inputString, Set<String> dict,
                               Set<String> unMatchedStrings, String answer) {
        if (inputString.length() == 0) {
            System.out.println(answer);
            return true;
        } else if (unMatchedStrings.contains(inputString)) {
            return false;
        } else {
            ...
            while (index < inputString.length()) {
                word += inputString.charAt(index);// add one char at a time
                // check if word already being solved
                if (dict.contains(word)) {
                    if (findUsingDP(inputString.substring(index + 1), dict, unMatchedStrings,
                            answer + word + " ")) {
                        return true;
                    } else {
            ...
            unMatchedStrings.add(inputString);// memoization for future;
            return false;
        }
    }
